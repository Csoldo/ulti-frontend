...
const GameSettings = ({ isOpen, onClose, onStartGame }: GameSettingsProps) => {
  // Mock data for existing players - in real app this would come from API
  const [existingPlayers] = useState<Player[]>([
    { id: 1, name: 'Anna' },
    { id: 2, name: 'Béla' },
    { id: 3, name: 'Csaba' },
    { id: 4, name: 'Dóra' },
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Játék beállítások">
      <div className={styles.container}>
        <div className={styles.section}>
          <h3 className={styles.sectionTitle}>Játékosok kiválasztása</h3>
        </div>

        {/* Selected Players */}
        {selectedPlayers.length > 0 && (
          <div className={styles.section}>
            <h4 className={styles.subsectionTitle}>Kiválasztott játékosok:</h4>
              ...
              <div className={styles.dropdownMenu}>
                {existingPlayers.map(player => (
                  <button
                    ...
                    onClick={() => handlePlayerToggle(player)}
                    disabled={!isPlayerSelected(player) && selectedPlayers.length >= 5}
                  >
                    <span className={styles.playerName}>{player.name}</span>
